...
class AllTimeVC: BaseViewController {
    ...
    @objc func handleTodayAnalysis()  {
        self.showLoading()
        ApiCalls.GetTodayAnalysis (report_type: 4) { (error: Error?, insights: [Insight]?, followRatio: [FollowRatio]?, ageGroupProfile: [AgeGroup]?, AgeGroupWows: [AgeGroup]?) in
            if error == nil {
