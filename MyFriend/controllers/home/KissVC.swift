...
class KissVC: BaseViewController {
    
    var initialTouchPoint:CGPoint = CGPoint(x: 0, y: 0)
    
    @IBOutlet weak var likesTable: UITableView!
    @IBOutlet weak var kissTotal: UILabel!
    static var id: Int = 0
    ...
    @objc func handlefollowersRefresh()  {
        ...
        isLoading = true
        print(KissVC.id)
        ApiCalls.getMyKisses(gallery_id: KissVC.id){ (error: Error?, randomImages: [RandomUser]?, last_page: Int) in
    ...
    fileprivate func LoadMore() {
        ...
        isLoading = true
        ApiCalls.getMyKisses(page: current_page+1, gallery_id: KissVC.id) { (error: Error?, randomImages: [RandomUser]?, last_page: Int) in
            self.isLoading = false
            if let randomImages = randomImages{
...
extension KissVC :UITableViewDataSource,UITableViewDelegate{
